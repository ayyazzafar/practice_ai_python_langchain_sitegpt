...
def main():
    if len(st.session_state) == 0:
        ...
        if is_openai_api_key_set():
            st.session_state["webquery"] = WebQuery(
    ...
    st.text_input("Input url", value=st.session_state["url"], key="input_url",
                  disabled=not is_openai_api_key_set(), on_change=ingest_input)
